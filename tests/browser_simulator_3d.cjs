...
(async()=>{
 ...
 try {
  ...
  await page.locator('#simPeopleVisibility').click();await page.locator('#simAddPerson').click();assert.equal((await snapshot()).people.length,3);
  await page.locator('#resetSimulation').click();assert.equal((await snapshot()).people.length,2);
