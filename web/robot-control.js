(() => {
  ...
  function parseTelemetry(line) {
    ...
    else if (values.STATE === "SENSOR_FAIL" && firmwareMode === 1 && !["operator", "fault", "transition"].includes(emergencyOwner)) {
    ...
    patch("robot", {
      connected,
      status: splitBrainHandling || values.STATE === "SENSOR_FAIL" && firmwareMode === 1 ? "ERROR" : connected ? "ONLINE" : "CONNECTING",
    ...
    patch("safety", {
      ...
      status: values.STATE === "SENSOR_FAIL" && firmwareMode === 1 ? "SENSOR_FAIL" : emergencyActive || values.STATE === "ESTOP" ? "EMERGENCY" : "MONITORING",
      obstacle,
    }, { source: "arduino-telemetry" });
  }
